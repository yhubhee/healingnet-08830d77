...
export async function listOrders_visible(): Promise<LabOrder[]> {
  const rows = unwrap(
    await backend.from("lab_results").select(ORDER_SELECT).order("created_at", { ascending: false }),
  );
  return (rows || []).map(toLabOrder);
}
...
export async function getOrder(orderId: string): Promise<LabOrder | null> {
  const row = unwrap(
    await backend.from("lab_results").select(ORDER_SELECT).eq("id", orderId).maybeSingle(),
  );
