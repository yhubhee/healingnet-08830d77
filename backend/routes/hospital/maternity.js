...
// GET /hospital/maternity - Render EJS
router.get('/', requireHospitalAuth, async (req, res) => {
  try {
    ...
    res.render('hospital/maternity', { records, stats: stats || {} });
  } catch (err) {
    console.error('Maternity error:', err);
    res.status(500).send('<div class="error-message"><h2>Failed to load maternity</h2><p>' + err.message + '</p></div>');
  }
});

// POST /hospital/maternity
router.post('/', requireHospitalAuth, async (req, res) => {
  try {
    const { patient_id, doctor_id, lmp_date, edd, gestational_age_weeks, gravida, para, risk_level, blood_group, genotype, hiv_status, hepatitis_b, notes } = req.body;
    if (!patient_id) return res.status(400).json({ success: false, message: 'Patient ID required.' });
    const result = await query(
      `INSERT INTO maternity_records (hospital_id, patient_id, doctor_id, lmp_date, edd, gestational_age_weeks, gravida, para, risk_level, blood_group, genotype, hiv_status, hepatitis_b, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [req.hospitalId, patient_id, doctor_id || null, lmp_date || null, edd || null, gestational_age_weeks || null, gravida || 1, para || 0, risk_level || 'low', blood_group || null, genotype || null, hiv_status || 'unknown', hepatitis_b || 'unknown', notes || null]
    );
    res.status(201).json({ success: true, id: result.insertId, message: 'Record created.' });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Server error.' });
  }
});

// PUT /hospital/maternity/:id
router.put('/:id', requireHospitalAuth, async (req, res) => {
  try {
    const allowedFields = ['status', 'risk_level', 'gestational_age_weeks', 'delivery_date', 'delivery_type', 'baby_weight', 'baby_gender', 'apgar_score', 'complications', 'notes', 'doctor_id'];
    const updates = [];
    const params = [];
