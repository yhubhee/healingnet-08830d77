...
// GET /hospital/consultations - Render EJS
  ...
  try {
    ...
    let sql = `
      SELECT cr.*,
             ...
             d.specialty AS doctor_specialty, d.profile_image_url AS doctor_image,
    ...
    const counts = await queryOne(`
      SELECT COUNT(*) AS total, SUM(status = 'pending') AS pending,
             SUM(status = 'accepted') AS accepted, SUM(status = 'completed') AS completed,
             SUM(status = 'rejected') AS rejected, SUM(status = 'cancelled') AS cancelled
      FROM consultation_requests WHERE requesting_hospital_id = ?
    `, [req.hospitalId]);

    res.render('hospital/consultations', { consultations, counts: counts || {} });
  } catch (err) {
    console.error('List consultations error:', err);
    res.status(500).send('<div class="error-message"><h2>Failed to load consultations</h2><p>' + err.message + '</p></div>');
  }
});

// GET /hospital/consultations/:id
  ...
  try {
    const consultation = await queryOne(`
      SELECT cr.*, d.first_name AS doctor_first_name, d.last_name AS doctor_last_name,
             ...
             p.firstname AS patient_firstname, p.lastname AS patient_lastname,
             p.email AS patient_email, p.phone AS patient_phone, p.date_of_birth, p.gender, p.blood_type,
             h.name AS doctor_hospital_name
      FROM consultation_requests cr
      JOIN doctors d ON cr.doctor_id = d.doctor_id
      JOIN patients p ON cr.patient_id = p.patient_id
...
// POST /hospital/consultations
router.post('/', requireHospitalAuth, async (req, res) => {
  try {
    const { doctor_id, patient_id, specialty_needed, urgency, request_type, reason, patient_summary, preferred_date, preferred_time, fee_agreed } = req.body;
    if (!doctor_id || !patient_id || !reason) return res.status(400).json({ success: false, message: 'Doctor, patient, and reason required.' });

    const doctor = await queryOne('SELECT d.doctor_id FROM doctors d JOIN doctor_marketplace dm ON d.doctor_id = dm.doctor_id WHERE d.doctor_id = ? AND dm.is_available_for_external = TRUE', [doctor_id]);
    if (!doctor) return res.status(404).json({ success: false, message: 'Doctor not available.' });
    ...
    const result = await query(
      `INSERT INTO consultation_requests
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [req.hospitalId, doctor_id, patient_id, specialty_needed || null, urgency || 'moderate', request_type || 'virtual', reason, patient_summary || null, preferred_date || null, preferred_time || null, fee_agreed || null]
    );
    res.status(201).json({ success: true, consultation_id: result.insertId, message: 'Request sent.' });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Server error.' });
  }
});

// PUT /hospital/consultations/:id
router.put('/:id', requireHospitalAuth, async (req, res) => {
  try {
    const existing = await queryOne('SELECT * FROM consultation_requests WHERE id = ? AND requesting_hospital_id = ?', [req.params.id, req.hospitalId]);
...
// DELETE /hospital/consultations/:id
router.delete('/:id', requireHospitalAuth, async (req, res) => {
  try {
    const existing = await queryOne('SELECT * FROM consultation_requests WHERE id = ? AND requesting_hospital_id = ? AND status = "pending"', [req.params.id, req.hospitalId]);
    if (!existing) return res.status(404).json({ success: false, message: 'Only pending can be deleted.' });
