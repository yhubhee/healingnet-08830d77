...
// GET /api/hospital/surgery - List surgery records
router.get('/', requireHospitalAuth, async (req, res) => {
  try {
    ...
    if (date) { sql += ' AND s.scheduled_date = ?'; params.push(targetDate); }
...
// POST /api/hospital/surgery
router.post('/', requireHospitalAuth, async (req, res) => {
  try {
    const { patient_id, surgeon_id, anaesthetist_id, procedure_name, procedure_type, theatre_number,
            anaesthesia_type, scheduled_date, scheduled_time, pre_op_diagnosis, notes } = req.body;

    if (!patient_id || !surgeon_id || !procedure_name || !scheduled_date || !scheduled_time) {
      return res.status(400).json({ success: false, message: 'Required fields missing.' });
    }

    const result = await query(
      `INSERT INTO surgery_records (hospital_id, patient_id, surgeon_id, anaesthetist_id, procedure_name,
       procedure_type, theatre_number, anaesthesia_type, scheduled_date, scheduled_time, pre_op_diagnosis, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [req.hospitalId, patient_id, surgeon_id, anaesthetist_id || null, procedure_name,
       procedure_type || 'elective', theatre_number || null, anaesthesia_type || 'general',
       scheduled_date, scheduled_time, pre_op_diagnosis || null, notes || null]
    );
...
// PUT /api/hospital/surgery/:id/status
