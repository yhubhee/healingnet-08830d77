...
// GET /hospital/billing - Render billing EJS
router.get('/', requireHospitalAuth, async (req, res) => {
  try {
    ...
    let sql = `
      SELECT hb.*, p.firstname AS patient_first_name, p.lastname AS patient_last_name
    ...
    res.render('hospital/billing', { bills, totals: totals || {}, formatNaira });
  } catch (err) {
    console.error('Billing error:', err);
    res.status(500).send('<div class="error-message"><h2>Failed to load billing</h2><p>' + err.message + '</p></div>');
  }
});
...
router.post('/', requireHospitalAuth, async (req, res) => {
  try {
    ...
    if (!patient_id || !billing_type || !amount) return res.status(400).json({ success: false, message: 'Patient, billing type, and amount required.' });
